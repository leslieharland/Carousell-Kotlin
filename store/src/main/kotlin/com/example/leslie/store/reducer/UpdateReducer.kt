...
object UpdateReducer : Reducer<UpdateAction>() {
    ...
    fun reduceItemsCollection(action: UpdateAction, currentItems: List<EntityItem>): List<EntityItem> =
            when (action) {
                is ReorderItemsAction -> action.items
                else -> super.reduceItemCollection(action, currentItems)
            }

    fun shouldReduceItem(action: UpdateAction, currentItem: EntityItem): Boolean =
            when (action) {
                is UpdateItemAction -> action.T.localId == currentItem.localId
